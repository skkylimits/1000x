...
> **Context:** Bedrijfsbreed second-brain dat documentatiesite, wiki en interactief leersysteem combineert. Gebouwd op de **Nuxt UI docs-template** als baseline (markdown-rendering, prose-componenten, code-blocks, search, dark mode komen daaruit), met 1000x-specifieke aanpassingen en eigen features daarbovenop. Interactiepatronen mede afgeleid van boot.dev (client-side code-execution model).
...
**Cross-cutting constraints** die op alle features van toepassing zijn:

- **Toegankelijkheid** — WCAG 2.1 AA als minimum; elk muis- of scroll-gedrag heeft een toetsenbord-equivalent
- **Modulaire architectuur** — nieuwe panels en mini-apps kunnen worden toegevoegd zonder herstructurering
- **Telemetry-ready** — geen telemetry in v1, maar de architectuur is zo opgezet dat het in latere fases standaard meegenomen wordt (niet opt-in — dit is een werkomgeving, geen consumer-product)

Voor proces, PR-discipline en de drie soorten werk (foundation / customizations / features): zie `CONTRIBUTING.md`.
...
- Hoofdstukken, pagina's, levels en tabs kunnen vanuit de UI worden aangemaakt — geen codebase-aanpassing nodig om content uit te breiden
- Edits en structuur-mutaties werken in de eerste release zonder backend, account of netwerk
- Werkt offline en is installeerbaar als PWA op desktop en mobiel

---

## User Roles

> Toegang tot het systeem vereist authenticatie in alle deployed omgevingen (zie Toegang en private deployment, feature 09 in 03-FEATURES). De rol "Lezer" beschrijft wat een gebruiker kan zodra hij door die auth-gate is. In lokale development is geen auth nodig.
...
> Geordend volgens de drie-stage roadmap uit `FEATURES.md`: foundation eerst, dan template-customizations, dan eigen features. SPEC.md beschrijft het _wat_ engine-agnostisch; de individuele `SPEC.md` per folder beschrijft het _hoe_. Cross-references gebruiken het folder-formaat (`customization N in 02-TEMPLATE`, `feature N in 03-FEATURES`) zodat ze blijven kloppen ongeacht hernummering hier.
...
> Stage 2 in `FEATURES.md`: aanpassingen op wat de Nuxt UI docs-template en Nuxt Content out-of-the-box leveren. De tien items hieronder mappen op de dertien folder-customizations onder `02-TEMPLATE/`. Sommige SPEC-features dekken meerdere folders (Right panel + smart ToC = customizations 07–08; Changelog + prev/next = customizations 09–10), en de Tabs-functionaliteit (folder 05-tabs) zit hier ingebed in Markdown rendering.

### 1. Markdown rendering & content-engine

→ detail-specs: [`02-TEMPLATE/01-markdown-rendering/SPEC.md`](./02-TEMPLATE/01-markdown-rendering/SPEC.md), [`02-TEMPLATE/05-tabs/SPEC.md`](./02-TEMPLATE/05-tabs/SPEC.md)

- Server-side rendering via Nuxt Content
- Headings H1–H4 met anchor-links
- Code blocks met syntax highlighting
- **Tabs** in content (Tailwind-stijl) zodat lange pagina's gecondenseerd kunnen worden zonder de file op te splitsen. Iedere tab heeft eigen H2/H3-koppen; de ToC reageert hierop (zie Right panel + smart ToC, customizations 07–08 in 02-TEMPLATE). **Overflow**: meer tabs dan in beeld passen scrollen horizontaal — touch-swipe, trackpad-scroll en shift+scrollwheel werken native. Een subtiele schaduw aan de zichtbare rand signaleert dat er meer is. Zie In-app content management, feature 02 in 03-FEATURES voor hoe het `+`-icoontje bereikbaar blijft tijdens scroll.
...
→ detail-spec: [`02-TEMPLATE/02-section-sidebar/SPEC.md`](./02-TEMPLATE/02-section-sidebar/SPEC.md)

Eén bron voor alle navigatie-surfaces — sidebar, header-dropdowns (zie Header, customization 03 in 02-TEMPLATE), prev/next (zie Changelog + prev/next, customizations 09–10 in 02-TEMPLATE), tab-bewuste smart ToC (zie Right panel + smart ToC, customizations 07–08 in 02-TEMPLATE), AppLevelHeader (zie Levels, customization 04 in 02-TEMPLATE), én de **breadcrumb** bovenaan de pagina-content (sectie / hoofdstuk / pagina). Geen `.navigation.yml`-bestanden, geen hardcoded arrays in components.
...
3. Merge van localStorage tree-mutations uit In-app content management (feature 02 in 03-FEATURES) — alleen client-side
...
**Header-dropdowns** lezen uit dezelfde boom — frontmatter levert icon en description per child. Een nieuw hoofdstuk toevoegen via In-app content management (feature 02 in 03-FEATURES) verschijnt automatisch in het Syntax-dropdown; geen handmatige sync met een hardcoded array.

**Prev/Next** wordt afgeleid uit de afgevlakte sidebar-boom in volgorde van de `nav`-array. **Level-bewust** komt automatisch via URL-nesting: omdat levels folder-based zijn (`/syntax/javascript/junior/...`), blijft prev/next vanzelf binnen de actieve level — geen variant-aware wrapper nodig.

**Levels en tabs zijn folder-/file-based, geen file-suffix-collapse**. Een directory met `levels: true` (of `levels: [...]`) op zijn `index.md` is een levels-container; zijn directe sub-folders zijn de levels (Junior/Mid/Senior, Windows/Linux/macOS). Een directory met `tabs: true` is een tabs-container; zijn directe sibling-files zijn de tabs (Vite/PostCSS/CLI). De tree krijgt kind-detectie per node (`page` / `chapter` / `levels-container` / `tabs-container` / `level` / `tab`); de sidebar toont onder een levels-container alleen de pagina's van de actieve level (de levels-container zelf is geen sidebar-entry — hij is chrome-navigatie via AppLevelHeader, zie Levels, customization 04 in 02-TEMPLATE). Switching tussen levels gaat via URL (`/syntax/javascript/senior/...`), niet via querystring — links zijn ondubbelzinnig deelbaar.

**Sidebar-rendering — consistente indentatie** ongeacht of een scope chapters heeft of niet:

- **Scope met chapters** (bv. JavaScript met _Variables / Functions / Closures / Async / Modules_): scope-label bovenaan met icon. Daaronder de chapters als rijen met het chapter-icon links en de **chevron aan de rechterkant** van de rij (niet naast het icon). Een uitgevouwen chapter rendert zijn pages in een ingesprongen container; de chevron roteert 90° om de open-state aan te geven.
...
**UI-state** (sidebar-collapse, gekozen panel, gekozen tab) is gebruiker-voorkeur, geen content-config. Woont in localStorage onder `ui:{key}` (zie Settings, feature 03 in 03-FEATURES). Vanaf Phase 3 verhuist deze state naar het gebruikersprofiel.
...
→ detail-spec: [`02-TEMPLATE/03-header/SPEC.md`](./02-TEMPLATE/03-header/SPEC.md)
...
3. **Kitt** — toolkits & commando's (git, nmap, wireshark, etc.). Pagina's leggen tools uit met hun belangrijkste flags en use-cases; waar mogelijk komt er een interactieve **terminal-emulator** in de panel-switcher zodat commando's direct te oefenen zijn — vergelijkbaar met de code editor (zie Code-editor, feature 04 in 03-FEATURES), maar dan met een sandboxed shell.
4. **Vuln** — vulnerabilities / security topics
5. **Xpl01ts** — exploits / write-ups
6. **Knowledge Base** — algemene wiki

De sub-items in elke dropdown zijn data-driven uit de content-tree, zodat in-app aangemaakte hoofdstukken (zie In-app content management, feature 02 in 03-FEATURES) automatisch in het menu verschijnen.
...
2. **AI** — opent slide-panel chatbot (zie AI assistent, feature 10 in 03-FEATURES)
3. **i18n** — taalwissel
4. **Light / Dark toggle**
5. **Settings**
...
### 4. Sub-header met levels (AppLevelHeader)

→ detail-spec: [`02-TEMPLATE/04-levels/SPEC.md`](./02-TEMPLATE/04-levels/SPEC.md)

Een generieke chrome-sub-header onder de hoofd-header voor secties die meerdere **levels** van dezelfde topic aanbieden. Wat een level betekent verschilt per sectie:
...
Een directory wordt een levels-container door `levels: true` (boolean) of `levels: [<slug>, ...]` (geordende array) op zijn `index.md` te zetten. De directe children van die container zijn **folders**; iedere child-folder is een level met eigen `index.md` (verplicht: `title` + `icon`) en eigen sub-tree pagina's. Geen file-suffixes, geen content-collapse, geen querystring — de level leeft in zijn eigen URL-pad (`/syntax/javascript/junior/...`).

```yaml
# content/syntax/javascript/index.md — levels-container
---
title: JavaScript
...
```yaml
# content/syntax/javascript/junior/index.md — level
---
title: Junior
...
- AppLevelHeader rendert wanneer een ancestor van de huidige route een levels-container is. Iedere level toont met icon + label; de actieve level is altijd in icon + label zichtbaar.
- Klikken op een level navigeert naar `/<scope>/<level>/` — de level's eigen `index.md` opent als first page.
- **Sidebar onder een levels-container** toont alleen de pagina's van de actieve level; de levels-container-node zelf verschijnt niet als sidebar-entry (hij is chrome-navigatie, niet content-navigatie).
- **Volgorde**: bij `levels: [junior, mid, senior]` volgt AppLevelHeader die volgorde; bij `levels: true` zonder array valt het terug op alfabetisch op level-slug. `nav: [...]` op de levels-container's `index.md` wint over `levels` (consistent met de overall ordering-resolver uit Section sidebar, customization 02 in 02-TEMPLATE).
- **Persistence**: actieve level zit in de URL, geen querystring. Bij hernieuwd bezoek aan de scope-root zonder explicit level kan een redirect plaatsvinden naar de laatst-bezochte level (gebruikersvoorkeur via Settings, feature 03 in 03-FEATURES) of naar de eerste level uit de array.
- **Combinatie met tabs**: een level-folder mag eigen tabs-containers bevatten (zie Tabs binnen Markdown rendering, customization 01 in 02-TEMPLATE); levels en tabs zijn onafhankelijke flags op verschillende directories. Een directory mag NIET tegelijk `levels: true` én `tabs: true` hebben — build faalt op die combinatie.
- **Prev/next** binnen een level werkt automatisch: omdat de routes nested zijn, blijft Nuxt Content's `content-surround` natuurlijk binnen de level. Geen variant-aware wrapper nodig.
- **Overflow-gedrag — collapse, geen scroll**: als alle levels met icon + label binnen de container passen, wordt alles zo getoond. Als ze niet passen, **collapsen non-actieve levels naar icon-only**; de actieve level blijft altijd in icon + label zichtbaar zodat de gebruiker weet waar hij is. Bij hover/focus op een collapsed level verschijnt het label als tooltip. Dit gedrag is bewust **anders dan bij content-tabs** (zie Markdown rendering, customization 01 in 02-TEMPLATE; horizontaal scrollen): levels zijn een prominente navigatie-keuze waar alle opties in één oogopslag zichtbaar moeten zijn — content-tabs zijn serieel doorlees-secties waar scrollen acceptabel is. De `+`-knop voor _nieuwe level_ (zie In-app content management, feature 02 in 03-FEATURES) blijft daardoor altijd zichtbaar zonder sticky te hoeven zijn.
- **Visuele consistentie**:
  - Horizontale dividers tussen secties lopen **edge-to-edge** met de container — geen inset of whitespace aan de zijkanten. Geldt voor élke divider tussen hoofdsecties (header → AppLevelHeader → content). Als de inhoud een max-width heeft, zit de divider op de _outer_ container, niet op de inner-padded.
  - De **active-state-underline** van een geselecteerde level (zowel hier als in het hoofdmenu, zie Header, customization 03 in 02-TEMPLATE) **landt op de divider** van zijn container — niet zwevend erboven met een gat ertussen. Praktisch betekent dit: padding op de outer container is horizontaal-only, elke level-tab heeft zelf de vertical padding, en de active border-bottom overlapt de container's divider met een negatieve margin van 0.5px. Resultaat: de level-rij voelt visueel gefuseerd met de divider in plaats van drie horizontale lijnen vlak boven elkaar.

### 5. Pagina-actiebalk

→ detail-spec: [`02-TEMPLATE/06-page-chrome/SPEC.md`](./02-TEMPLATE/06-page-chrome/SPEC.md) (samen met de breadcrumb uit Section sidebar, customization 02 in 02-TEMPLATE)
...
> Toekomstige dropdown-opties: **_Genereer PowerPoint_** en **_Genereer documentatie_** — beide leveren een download op basis van de pagina-inhoud. Beide leunen op de AI-laag (zie AI assistent, feature 10 in 03-FEATURES) en zijn dus pas zinvol nadat de LLM-integratie staat.

### 6. Rechter panel met conditionele panel-switcher en smart ToC

→ detail-specs: [`02-TEMPLATE/07-right-panel/SPEC.md`](./02-TEMPLATE/07-right-panel/SPEC.md), [`02-TEMPLATE/08-smart-toc/SPEC.md`](./02-TEMPLATE/08-smart-toc/SPEC.md)
...
- **Tab-bewuste ToC**: als de pagina content-tabs heeft (zie Markdown rendering, customization 01 in 02-TEMPLATE), reflecteert de ToC de koppen van de **actief geselecteerde tab**. Switch je van tab, dan herrendert de ToC. Anchors springen alleen naar koppen binnen de actieve tab — zo verwijzen ToC-links nooit naar verborgen content.
- **Open / gesloten staat**: het panel is standaard open op desktop. Een toggle in de actiebalk (of via een toetsenbordcombinatie) sluit het volledig — niet alleen smaller; de content-kolom krijgt de vrijgekomen breedte. De gekozen staat (open/dicht, breedte, actieve view) wordt onthouden via `ui:{key}` in localStorage (zie Settings, feature 03 in 03-FEATURES), vanaf Phase 3 in het gebruikersprofiel.
- **Default-breedte** ~280px met een minimum van ~220px en een maximum van ~480px via de resize-handle. Daarboven schiet de content-kolom te smal voor comfortabel lezen.
- **Switch tussen views** is instant: de panel-container blijft staan, alleen de inhoud verwisselt. Geen kruislingse fade of slide-animatie tussen panels — dat leidt af van de actie die de gebruiker net deed (een knop ingedrukt, een toets ingedrukt). De resize-handle blijft op zijn plek bij elke switch.

### 7. Changelog en navigatie onderaan pagina

→ detail-specs: [`02-TEMPLATE/09-changelog/SPEC.md`](./02-TEMPLATE/09-changelog/SPEC.md), [`02-TEMPLATE/10-prev-next/SPEC.md`](./02-TEMPLATE/10-prev-next/SPEC.md)

- **Changelog** direct onder de content: commit/edit-history van die pagina in Nuxt UI Pro changelog-stijl — een verticale timeline gegroepeerd per versie. Per versie een tag-icoon en versie-badge met datum; per commit eronder een hash-chip, auteur (avatar + naam), commit-bericht en optioneel een PR-nummer als link. Eén doorlopende verticale lijn loopt door alle items (versions én commits) — zelfde "één lijn"-regel als de sidebar.
...
→ detail-spec: [`02-TEMPLATE/11-search/SPEC.md`](./02-TEMPLATE/11-search/SPEC.md)

- Cmd/Ctrl+K opent een command palette à la Nuxt UI's `CommandPalette`. Ook bereikbaar via het search-icoon in de header (zie Header, customization 03 in 02-TEMPLATE).
...
- **Latere iteraties** beantwoorden expliciet: wat geïndexeerd wordt (titels, headings, body, code-blocks), gedrag voor i18n (multi-locale of per-taal), offline-zoeken in PWA-context, en hoe in-app aangemaakte pagina's (zie In-app content management, feature 02 in 03-FEATURES) in de zoekindex landen. Een robuustere search (bv. Pagefind of MiniSearch met eigen index) is een upgrade-pad zodra de inhoud groeit.

### 9. Wiskundige formules en diagrammen

→ detail-spec: [`02-TEMPLATE/12-math-en-diagrammen/SPEC.md`](./02-TEMPLATE/12-math-en-diagrammen/SPEC.md)

- **KaTeX** voor wiskundige formules in markdown — `$inline$` voor inline, `$$display$$` voor block-niveau. Gebruikt voor cryptografie-formules, complexity-analyse, en algoritmische beschrijvingen in security/CS-content.
- **Mermaid** voor diagrammen in markdown — code blocks met language `mermaid` (flowcharts, sequence diagrams, system diagrams, ER-diagrammen). Gebruikt voor architectuur-uitleg, attack-flows, request-lifecycles.
- KaTeX boven MathJax: kleiner, sneller, voldoende voor 95% van de wiskunde die we hier nodig hebben.
- Beide zijn **opt-in per pagina** — laad de runtime alleen wanneer een pagina daadwerkelijk een formule of diagram bevat (frontmatter-vlag of content-detectie). Geen page-payload-bloat voor pagina's zonder formules.
- Aparte feature van Markdown rendering (customization 01 in 02-TEMPLATE) zodat het bewust opt-in is en niet stiekem in de baseline kruipt.

### 10. Image lightbox

→ detail-spec: [`02-TEMPLATE/13-image-lightbox/SPEC.md`](./02-TEMPLATE/13-image-lightbox/SPEC.md)

- Klik op een afbeelding in een artikel opent een fullscreen overlay met de afbeelding op volledige resolutie. Sluiten via `Esc`, klik buiten de afbeelding, of een sluit-knop.
- Markdown alt-text wordt gebruikt als caption in de lightbox-view.
- Skip voor v1: multi-image gallery met thumbnails / arrow-key navigation tussen images, pinch-to-zoom op mobiel, image-annotatie tools.
- Voor screenshots van VS Code, terminal-output, network-diagrammen — alles waar lezers op willen inzoomen om details te zien.
- Aparte feature zodat de scope (caption-styling, gallery-gedrag, mobile interactions) bewust afgebakend kan worden in plaats van mee te liften op rendering.

---

## Eigen features

> Stage 3 in `FEATURES.md`: tien echte features die de Nuxt UI docs-template niet levert. Brengen vaak nieuwe libraries mee (CodeMirror, Pyodide, etc.) en eigen state-management (Pinia komt binnen bij feature 02 — zie `ARCHITECTURE.md`).

### 1. View / Edit-toggle met lokale drafts

→ detail-spec: [`03-FEATURES/01-edit-met-drafts/SPEC.md`](./03-FEATURES/01-edit-met-drafts/SPEC.md)

- Bewerk-knoppen zitten op de actiebalk **naast de paginatitel** (zie Pagina-actiebalk, customization 06 in 02-TEMPLATE).
...
- **Quota & data-loss safety net**:
  ...
  - **Settings-knop _Export drafts_** — downloadt alle drafts én lokale tree-mutaties (zie In-app content management, feature 02 in 03-FEATURES) als één ZIP, in een gedocumenteerd JSON-formaat (versie + payload).
...
- **Latere fase — IndexedDB**: zodra drafts groter worden, in-app content management groeit (zie feature 02 in 03-FEATURES) en/of we offline-first willen zijn over meer dan alleen reads, migreren we van localStorage naar **IndexedDB**. Sleutels en datamodel blijven hetzelfde; alleen de storage-laag wordt vervangen.
...
### 2. In-app content management

→ detail-spec: [`03-FEATURES/02-content-management/SPEC.md`](./03-FEATURES/02-content-management/SPEC.md)

Doel: nieuwe **hoofdstukken, pagina's, levels en content-tabs** kunnen aanmaken zonder de codebase te openen. Alles wat structureel is moet via de UI kunnen — de markdown-editor blijft alleen voor de inhoud zelf.
...
- Naast de **levels in AppLevelHeader** (zie Levels, customization 04 in 02-TEMPLATE) → voor een nieuwe level. Maakt een nieuwe level-folder aan onder de levels-container met een eigen `index.md` (en optioneel een eerste pagina overgenomen van een bestaande level), zodat je niet vanaf nul begint.
- Naast de **content-tabs in de pagina** (zie Markdown rendering, customization 01 in 02-TEMPLATE) → voor een nieuwe tab. De nieuwe tab is een sibling-file onder de tabs-container; tab-bestanden worden automatisch herkend via het `tabs: true`-flag op de container's `index.md`. De nieuwe tab opent direct in edit-mode klaar voor inhoud.
...
In beide gevallen: klik → kleine inline input voor de naam → enter → het nieuwe item verschijnt direct in de UI; bij een pagina/level/tab opent die meteen in edit-mode (zie View / Edit-toggle, feature 01 in 03-FEATURES). Rechts-klik op een bestaand item → context-menu met **rename** en **delete**.
...
- **Fase 3 — IndexedDB**: zodra structuur-mutaties en drafts samen significant groeien, migreert de overlay-storage naar IndexedDB (gelijktijdig met View / Edit, feature 01 in 03-FEATURES).
...
→ detail-spec: [`03-FEATURES/03-settings/SPEC.md`](./03-FEATURES/03-settings/SPEC.md)

Eigen pagina/dialog, ontworpen in **kaart-stijl** geïnspireerd op certificates.dev — meerdere kaarten in een grid, elke kaart één onderwerp. Vormgeving volgt de huisstijl van het 1000x-systeem zelf, niet die van certificates.dev.

Inhoud (eerste release):

- **Voorkeurstaal** — NL / EN, koppelt aan i18n (zie i18n, customization 02 in 01-FOUNDATION)
...
- **Drafts beheren** — `Export drafts` (ZIP), `Import drafts` (ZIP), en _Wis alle lokale drafts_ met bevestigingsdialog. Dekt View / Edit (feature 01 in 03-FEATURES) en In-app content management (feature 02 in 03-FEATURES).
...
**Onthouden UI-state**: panel-keuze, sidebar collapse-state, laatst-bezochte level per levels-container (bv. Junior/Mid/Senior op een JS-pagina, voor de redirect-naar-laatste-level uit Levels, customization 04 in 02-TEMPLATE), gekozen content-tab — opgeslagen in localStorage onder `ui:{key}` in de eerste release. Vanaf Phase 3 (IAM/SSO/SCIM/Entra) verhuist deze state naar het gebruikersprofiel zodat hij over apparaten heen synct.

### 4. Code-editor met browser-based execution

→ detail-spec: [`03-FEATURES/04-code-editor/SPEC.md`](./03-FEATURES/04-code-editor/SPEC.md)

- De code editor leeft als panel-optie in de panel-switcher (zie Right panel + smart ToC, customizations 07–08 in 02-TEMPLATE), **niet** als losse knoppenrij boven de changelog. De editor-knop verschijnt alleen op pagina's met een programmeertaal of code-use-case.
...
### 5. Card trainer (flashcards / quiz / exam)

→ detail-spec: [`03-FEATURES/05-card-trainer/SPEC.md`](./03-FEATURES/05-card-trainer/SPEC.md)
...
- Auto-generatie uit pagina-inhoud (revisely-stijl) volgt later, gebruikt de AI-laag uit AI assistent (feature 10 in 03-FEATURES).

### 6. Comment-systeem

→ detail-spec: [`03-FEATURES/06-comments/SPEC.md`](./03-FEATURES/06-comments/SPEC.md)
...
→ detail-spec: [`03-FEATURES/07-mobiele-layout/SPEC.md`](./03-FEATURES/07-mobiele-layout/SPEC.md)
...
- **Card trainer overlay** (zie Card trainer, feature 05 in 03-FEATURES): op mobiel volledig scherm i.p.v. 70%.
...
### 8. PWA en offline-werking

→ detail-spec: [`03-FEATURES/08-pwa-offline/SPEC.md`](./03-FEATURES/08-pwa-offline/SPEC.md)
...
- Drafts (zie View / Edit, feature 01 in 03-FEATURES) en lokale tree-mutaties (zie In-app content management, feature 02 in 03-FEATURES) zitten al in localStorage en werken sowieso offline.
- Code-execution (zie Code-editor, feature 04 in 03-FEATURES) draait client-side en blijft offline werken zodra de WASM-runtime is gecached.

### 9. Toegang en private deployment

→ detail-spec: [`03-FEATURES/09-toegang-private-deployment/SPEC.md`](./03-FEATURES/09-toegang-private-deployment/SPEC.md)
...
### 10. AI assistent in slide-panel

→ detail-spec: [`03-FEATURES/10-ai-assistent/SPEC.md`](./03-FEATURES/10-ai-assistent/SPEC.md)
...
- **Latere uitbreidingen**:
  ...
  - **Generatie-acties** vanuit de pagina-actiebalk dropdown (zie Pagina-actiebalk, customization 06 in 02-TEMPLATE): _Genereer PowerPoint_, _Genereer documentatie_
...
- **Baseline** — [Nuxt UI docs-template](https://github.com/nuxt-ui-templates/docs) als startpunt. Levert de fundamenten van de docs-rendering (markdown, prose, code blocks, search, dark mode, auto-sidebar) zonder dat we ze zelf bouwen. Onze customizations en features bouwen daarbovenop. Zie `SCAFFOLDING.md` voor de stap-voor-stap setup.
- **Framework** — Nuxt 4 (Nuxt 3 is end-of-life op 31 juli 2026; nieuwe projecten starten op v4)
- **UI-componenten** — Nuxt UI v4 (de unified open-source library, voorheen Nuxt UI Pro). 100+ componenten inclusief layout (header, dashboard, navigation), content (`UContentSurround`, `UContentToc`, `UContentSearch`, `UContentNavigation`), interactie (tabs, breadcrumbs, command palette, modals, toast), feedback (alerts, callouts, banners), data-display (cards, accordions, collapsibles, tables) plus de prose-componenten voor markdown. **Verplichte volgorde voor élk UI-element**: (1) eerst checken of Nuxt UI v4 het levert — als component én als MDC-block waar van toepassing; (2) pas als het niet bestaat of fundamenteel onvoldoende is, een eigen component bouwen door een Nuxt UI component te slot-overriden of te wrappen; (3) volledig from-scratch bouwen alleen als laatste optie. "Niet bestaat" betekent letterlijk niet bestaat — niet "bestaat maar ik wil iets anders". Smaak-verschillen los je op met theming via `app.config.ts` (`ui.{component}.slots`), niet met een eigen herimplementatie
...
- **Images** — `@nuxt/image` als image-pipeline vanaf het begin met de **`ipx` provider** (lokaal optimaliseren via Nitro server). Werkt zowel voor SSG als voor Docker-deployment, hostingplatform-agnostisch. Levert automatische resizing per breakpoint, format-conversie naar WebP/AVIF, en lazy-loading. Direct opnemen voorkomt een latere refactor waarbij alle markdown-images en `ProseImg`-instances aangepast moeten worden — de pipeline is invisible plumbing onder Markdown rendering (customization 01 in 02-TEMPLATE), niet een eigen feature. Latere migratie naar een edge-provider (Cloudflare, Vercel) is een config-wijziging, geen codewijziging
...
- **Persistentie eerste release** — browser-localstorage voor zowel pagina-drafts (zie View / Edit, feature 01 in 03-FEATURES) als sidebar tree-mutaties (zie In-app content management, feature 02 in 03-FEATURES)
