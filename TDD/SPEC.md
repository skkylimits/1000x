...
- Hoofdstukken, pagina's, varianten en tabs kunnen vanuit de UI worden aangemaakt — geen codebase-aanpassing nodig om content uit te breiden
...
Eén bron voor alle navigatie-surfaces — sidebar, header-dropdowns (feature 3), prev/next (feature 6), tab-bewuste smart ToC (feature 5), levels-collapse (feature 8), én de **breadcrumb** bovenaan de pagina-content (sectie / hoofdstuk / pagina). Geen `.navigation.yml`-bestanden, geen hardcoded arrays in components.
...
3. Merge van localStorage tree-mutations uit feature 11 — alleen client-side
...
**Header-dropdowns** lezen uit dezelfde boom — frontmatter levert icon en description per child. Een nieuwe taal toevoegen via feature 11 verschijnt automatisch in het Syntax-dropdown; geen handmatige sync met een hardcoded array.
...
**UI-state** (sidebar-collapse, gekozen panel, gekozen tab) is gebruiker-voorkeur, geen content-config. Woont in localStorage onder `ui:{key}` (feature 12). Vanaf Phase 3 verhuist deze state naar het gebruikersprofiel.
...
3. **Kitt** — toolkits & commando's (git, nmap, wireshark, etc.). Pagina's leggen tools uit met hun belangrijkste flags en use-cases; waar mogelijk komt er een interactieve **terminal-emulator** in de panel-switcher zodat commando's direct te oefenen zijn — vergelijkbaar met de code editor in feature 13, maar dan met een sandboxed shell.
4. **Vuln** — vulnerabilities / security topics
5. **Xpl01ts** — exploits / write-ups
6. **Knowledge Base** — algemene wiki

De sub-items in elke dropdown zijn data-driven uit de content-tree, zodat in-app aangemaakte hoofdstukken (feature 11) automatisch in het menu verschijnen.
...
2. **AI** — opent slide-panel chatbot (zie feature 19)
3. **i18n** — taalwissel
4. **Light / Dark toggle**
5. **Settings**
...
- Localstorage-drafts en lokale tree-mutaties (features feature 10 en feature 11) per taal apart: sleutel bevat `{lang}`
...
> Toekomstige dropdown-opties: **_Genereer PowerPoint_** en **_Genereer documentatie_** — beide leveren een download op basis van de pagina-inhoud. Beide leunen op de AI-laag (zie feature 19) en zijn dus pas zinvol nadat de LLM-integratie staat.
...
- **Latere iteraties** beantwoorden expliciet: wat geïndexeerd wordt (titels, headings, body, code-blocks), gedrag voor i18n (multi-locale of per-taal), offline-zoeken in PWA-context, en hoe in-app aangemaakte pagina's (feature 11) in de zoekindex landen. Een robuustere search (bv. Pagefind of MiniSearch met eigen index) is een upgrade-pad zodra de inhoud groeit.

### 10. View / Edit-toggle met lokale drafts

- Bewerk-knoppen zitten op de actiebalk **naast de paginatitel** (zie feature 6).
...
- **Quota & data-loss safety net**:
  ...
  - **Settings-knop _Export drafts_** — downloadt alle drafts én lokale tree-mutaties (feature 11) als één ZIP, in een gedocumenteerd JSON-formaat (versie + payload).
...
- **Latere fase — IndexedDB**: zodra drafts groter worden, in-app content management groeit (feature 11) en/of we offline-first willen zijn over meer dan alleen reads, migreren we van localStorage naar **IndexedDB**. Sleutels en datamodel blijven hetzelfde; alleen de storage-laag wordt vervangen.
...
In beide gevallen: klik → kleine inline input voor de naam → enter → het nieuwe item verschijnt direct in de UI; bij een pagina/variant/tab opent die meteen in edit-mode (feature 10). Rechts-klik op een bestaand item → context-menu met **rename** en **delete**.
...
- **Fase 3 — IndexedDB**: zodra structuur-mutaties en drafts samen significant groeien, migreert de overlay-storage naar IndexedDB (gelijktijdig met feature 10).
...
- **Voorkeurstaal** — NL / EN, koppelt aan i18n (feature 4)
...
- **Drafts beheren** — `Export drafts` (ZIP), `Import drafts` (ZIP), en _Wis alle lokale drafts_ met bevestigingsdialog. Dekt feature 10 en 3.
...
- De code editor leeft als panel-optie in de panel-switcher (feature 5), **niet** als losse knoppenrij boven de changelog. De editor-knop verschijnt alleen op pagina's met een programmeertaal of code-use-case.
...
- Auto-generatie uit pagina-inhoud (revisely-stijl) volgt later, gebruikt de AI-laag uit feature 19.
...
- **Card trainer overlay** (feature 14): op mobiel volledig scherm i.p.v. 70%.
...
- Drafts (feature 10) en lokale tree-mutaties (feature 11) zitten al in localStorage en werken sowieso offline.
- Code-execution (feature 13) draait client-side en blijft offline werken zodra de WASM-runtime is gecached.
...
- **Latere uitbreidingen**:
  ...
  - **Generatie-acties** vanuit de pagina-actiebalk dropdown (feature 7): _Genereer PowerPoint_, _Genereer documentatie_
...
- Aparte feature van markdown rendering (feature 1) zodat het bewust opt-in is en niet stiekem in de baseline kruipt.
...
- **Persistentie eerste release** — browser-localstorage voor zowel pagina-drafts (feature 10) als sidebar tree-mutaties (feature 11)
