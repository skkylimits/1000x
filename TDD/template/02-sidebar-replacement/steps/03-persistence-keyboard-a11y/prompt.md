...
- `TDD/template/02-sidebar-replacement/SPEC.md` — this customization in detail. Read **§ Implementation Steps → Step 3** for this task's scope and **§ Constraints** for what stays out
- `TDD/features/12-settings/SPEC.md` — the future feature that will reuse the `settingsStore` interface introduced here. The interface must be designed so feature 12 can adopt it without rewriting Step 3's code
