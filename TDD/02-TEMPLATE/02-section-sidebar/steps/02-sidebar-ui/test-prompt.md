...
- Levels-container suppression as a render-level E2E test — the demo content has no levels-folders yet; the unit test from Step 1 already covers the kind-detection. A render-level test is deferred until `01-FOUNDATION/03-content-stubs` ships levels-folders. Mark this case as `test.fixme(...)` so the gap stays visible
- Tabs-container leaf rendering as a render-level E2E test — same reasoning. Mark as `test.fixme(...)` until tabs-content stubs exist
