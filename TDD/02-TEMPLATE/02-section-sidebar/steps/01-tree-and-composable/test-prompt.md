...
You should see "No test files found" and a non-zero exit. That is the baseline — your new files move the suite from "no tests" to a set of failing tests, each with its own assertion or runtime error, which is the correct red state.
...
- Order resolution: `nav: [...]` on the directory's `index.md` > `order: N` per page > alphabetical on `title`. Slugs not present in `nav` fall to the tail in alphabetical order. For levels-containers, `nav` wins over `levels: [...]`-array; for tabs-containers, `nav` wins over `tabs: [...]`-array
- Kind detection: a directory whose `index.md` has `levels: true | <array>` is `levels-container`; its direct child directories are `level`. A directory whose `index.md` has `tabs: true | <array>` is `tabs-container`; its direct children are `tab`. A directory with child pages and neither flag is `chapter`. A leaf page is `page`
- Validation: a directory node that is a scope-label (`scope === 'self'` or `scope === 'children'`) **must** have `icon`. A chapter (a directory node with at least one descendant page and no `levels`/`tabs` flag) **must** have `icon`. A `level` node **must** have `icon`. Missing → `buildTree` throws and the error message includes the offending path. Leaf pages without `icon` do **not** throw. A directory that declares both `levels` and `tabs` **must** throw with the offending path in the message
...
export function walkEffectiveScope(_routePath: string, _lookup: Map<string, NavNode>): NavNode | null {
	throw new Error('not implemented: walkEffectiveScope')
}
...
describe('buildTree — order resolver', () => {
	...
	it('respects nav over the levels: [...] array on a levels-container', () => {
		// levels-container with nav: [senior, mid, junior] and levels: [junior, mid, senior]
		// yields children in [senior, mid, junior]
	})

	it('uses the levels: [...] array order when no nav is set', () => {
		// levels-container with levels: [senior, mid, junior] (no nav) yields [senior, mid, junior]
	})
...
describe('buildTree — kind detection', () => {
	it('marks a levels-container directory and its level children', () => {
		// dir with levels: true → kind 'levels-container'
		// its child folders → kind 'level'
		// pages inside a level → kind 'page'
	})

	it('marks a tabs-container directory and its tab children', () => {
		// dir with tabs: true → kind 'tabs-container'
		// its direct children (file or folder) → kind 'tab'
	})

	it('marks a directory with child pages and no levels/tabs flag as chapter', () => {
		// dir without levels/tabs but with pages → kind 'chapter'
	})

	it('marks a leaf page as page', () => {
		// single .md file with no children → kind 'page'
	})
})
...
describe('buildTree — icon validation', () => {
	...
	it('throws when a level node lacks an icon', () => {
		// child folder of a levels-container, with index.md but no icon → throw matching /icon/
	})
...
describe('buildTree — flag conflict', () => {
	it('throws when a directory declares both levels and tabs', () => {
		// dir({ path: '/javascript', scope: 'self', icon: '...', levels: true, tabs: true })
		// → throw with /\/javascript/ in the message
	})
})
...
describe('walkEffectiveScope', () => {
	it('returns the active level when the resolved scope is a levels-container', () => {
		// route '/syntax/javascript/junior/closures' with /syntax/javascript = levels-container, scope: self
		// → returns the /syntax/javascript/junior level node
	})

	it('returns the same node as walkScope for a regular scope', () => {
		// route under a non-levels-container scope: 'self' directory → walkEffectiveScope === walkScope
	})

	it('returns the levels-container itself when the route is the container index.md', () => {
		// route '/syntax/javascript' with that being the levels-container → returns the container, not a level
	})
})
...
Be careful with the `_id` / `stem` fields when constructing fixtures: directory-vs-leaf detection works off the source filename. For a directory node, set `stem: 'index.md'`; for a leaf page, set `stem` to the simple filename (e.g. `'closures.md'`).
...
- ✅ `pnpm test` runs, discovers `tests/unit/nav.test.ts`, and reports every `it` block as **failing** at the assertion or runtime level (most on the scaffold-stub's `not implemented: <name>` throw; the icon-validation and flag-conflict tests fail on regex-mismatch or unexpected-throw against the stub) — that is the intended red state
...
- ❌ **Do not** assert on internal implementation details (e.g. the exact rule used to walk down to the active level); test behaviour only
- ❌ **Do not** test any file-suffix `<base>.<variant>.<lang>.md` collapse — that pattern is deprecated. Levels are folders; tabs are sibling files; both are detected via the `levels`/`tabs` frontmatter flags on a directory's `index.md`
